...
bot.on('playerJoin', async (user) => {
  ...
  if (user.username === 'Atekinz') {
    bot.message.send("Mi creator se unió a la sala @Atekinz ❤️")
  }
...
bot.on('playerJoin', (user) => {
  ...
  bot.message.send(`¿Qué tal divertirte en la pista de baile? @${user.username}!`);
});
...
bot.on('playerLeave', (user) => {
  ...
  bot.message.send(`Bye @${user.username}!`)
});
